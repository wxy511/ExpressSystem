 ...
 */
XSExport.sheet2blob = function(sheet, sheetName, color,colswidth) {
	var that = this;
	...
	// 设置样式
	that.setWorkbookStyle(workbook,workbook.SheetNames[0],color,colswidth);
 ...
 */
XSExport.setWorkbookStyle = function(wb,sheetName,color,colswidth){			
	...
	XSU.setFontSizeAll(wb,sheetName,9);//字体大小：10
	XSU.setAlignmentHorizontalAll(wb,sheetName,'left');//垂直居中
	...
	if(colswidth != ''){
	XSU.setColWidth(wb,sheetName,colswidth);
	}
	else{
		XSU.setColWidthAll(wb,sheetName,20)
	}
	XSU.mergeSameColCells(wb,sheetName,"A")
	...
	XSU.setSecondRowStylesDefault(wb,sheetName,color);
 ...
 */
XSExport.excelExport = function(data,title,color,colswidth){	
	var that = this;
	var aoa = data;	
	// 插入头部
	var header = [];
	...
	// 二进制文件
	var blob = that.sheet2blob(sheet,'sheet1',color,colswidth);
	
	that.openDownloadDialog(blob, title+'.xlsx');
}
/**
 * 日期对象转换为指定格式的字符串
