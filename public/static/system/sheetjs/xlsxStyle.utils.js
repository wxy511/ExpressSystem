...
XSU=({
	...
	*/
	...
	//设置第二行标题默认样式
	setSecondRowStylesDefault: function(workBook, sheetName,color) {
		for (var cell in workBook.Sheets[sheetName]) {
			var num= cell.replace(/[^0-9]/ig,"");
			if("2" === num){
				this.setFillFgColorRGB(workBook, sheetName, cell, color);
		...
		for(var i = 0;i<cells.length-1;){
			for(var j=i+1;j<cells.length;j++){
				...
				if(workBook.Sheets[sheetName][cells[i]].v == workBook.Sheets[sheetName][cells[j]].v && workBook.Sheets[sheetName][cells[i]].v != ""){
					if(j<cells.length-1 && workBook.Sheets[sheetName][cells[i]].v != workBook.Sheets[sheetName][cells[j + 1]].v && workBook.Sheets[sheetName][cells[i]].v != ""){
